...
// Brescia centro
const BRESCIA = { latitude: 45.5416, longitude: 10.2118, zoom: 13 }
...
function SearchBar({ onSelect }) {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        const url =
          ...
          `?access_token=${MAPBOX_TOKEN}&language=it&country=it` +
          `&types=address,place,locality,neighborhood&limit=6`
  ...
  return (
    <div ref={wrapperRef} style={{
      ...
      width: 'min(350px, 90vw)', fontFamily: 'system-ui, sans-serif',
    }}>
      <div style={{
        display: 'flex', alignItems: 'center', background: '#fff',
        borderRadius: open && results.length ? '12px 12px 0 0' : '12px',
        boxShadow: '0 2px 12px rgba(0,0,0,0.18)', padding: '6px 10px', gap: 8,
      }}>
        <input
      ...
      {open && results.length > 0 && (
        <ul style={{
          ...
          borderRadius: '0 0 12px 12px', boxShadow: '0 6px 16px rgba(0,0,0,0.15)',
...
// ── Mappa ────────────────────────────────────────────────────────────────────
function Mappa() {
  const { parcheggiFiltrati = [], modifyPosition, modifyZoom } = useStore()
  const [selectedParcheggio, setSelectedParcheggio] = useState(null)

  // Riferimento diretto alla mappa Mapbox — niente viewState in React state
  const mapRef = useRef(null)
  const syncTimerRef = useRef(null)

  // Sincronizza store solo quando l'utente finisce di muoversi (debounce)
  ...
  }, [modifyPosition, modifyZoom])

  // La searchbar usa flyTo direttamente sull'istanza Mapbox — zero re-render React
  const handleSearchSelect = useCallback(({ longitude, latitude }) => {
    mapRef.current?.flyTo({ center: [longitude, latitude], zoom: 16, duration: 800 })
  }, [])

  const handleMarkerClick = useCallback((p) => setSelectedParcheggio(p), [])
  const handlePopupClose = useCallback(() => setSelectedParcheggio(null), [])
  ...
  return (
    <div style={{ position: 'relative', height: '100%', width: '100%' }}>
      ...
      <Map
        ref={mapRef}
        // ↓ initialViewState invece di viewState controllato:
        //   la mappa gestisce il proprio stato internamente → nessun re-render React durante il drag
        initialViewState={{
          longitude: BRESCIA.longitude,
          latitude: BRESCIA.latitude,
          zoom: BRESCIA.zoom,
          pitch: 45,
          bearing: 0,
        }}
        onMoveEnd={handleMoveEnd}   // ← solo onMoveEnd, non onMove
